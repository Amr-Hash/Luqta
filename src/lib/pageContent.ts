 ...
 */
export function readerMarkdownToSnippet(raw: string): string | null {
  const decoded = decodeEntities(raw)
  const title =
    pickLabeled(decoded, [/^Title:\s*(.+)$/im, /^#\s+(.+)$/m]) ||
    null

  const cleaned = stripMarkdownNoise(decoded)
  if (!cleaned && !title) return null

  // Prefer an “about / description / نبذة” paragraph
  const about = pickLabeled(cleaned, [
    /(?:^|\n)(?:About(?: this item)?|Product description|Description|Overview|نبذة|الوصف|تفاصيل المنتج)\s*[:：]?\s*\n?([\s\S]{20,600}?)(?=\n(?:About|Description|نبذة|الوصف|Specifications|المواصفات|#|\*|$)|\n\n\n)/i,
  ])
